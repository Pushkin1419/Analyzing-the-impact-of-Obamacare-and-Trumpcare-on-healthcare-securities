...
nyseBiotech=read.csv("./Risk Management Project/Data/MarketCap/NyseBiotechMarketCap.csv", header=T)
head(nyseBiotech)
colnames(nyseBiotech)
str(nyseBiotech)

### 0.2 Let's make a quick check on our missing values in each column
temp=rep(0,9)
for (i in 1:dim(nyseBiotech)[2])
{
  temp[i]=sum(is.na(nyseBiotech[,i]))
}
temp

## STJ.US.Equity has a ton of missing values, therefore I am removing it
nyseBiotech=nyseBiotech[,-7]

## 0.3
#Converting Date Column into 'Date' type object 
nyseBiotech$Date=as.character(nyseBiotech$Date)
nyseBiotech$Date=mdy(nyseBiotech$Date)

#### 1. First window-> 2009-01-01 to 2013-05-31
### 1.1 Subsetting the range of dates we want
#Subsetting NyseBiotech in the date range I want
d1= as.Date("2009-01-01")
d2= as.Date("2013-06-01")
myTable=nyseBiotech[c(1050:2265),]
myTable= subset(nyseBiotech, nyseBiotech[,1] >= d1 & nyseBiotech[,1]<d2)
dim(myTable)
...
#Checking for Serial Correlation in Returns itself
acf(returns)
pacf(returns)
...
plot(d, vol1, type="l", xlab="Date", ylab="Volatility Estimate", main="NYSE Biotech Index")
